...
train_transform = v2.Compose([
    ...
    v2.Resize(256),
    v2.RandomResizedCrop(224, scale=(0.5, 1), ratio=(3/4,4/3)),
    v2.RandomHorizontalFlip(p=0.5),
    #v2.AutoAugment(AutoAugmentPolicy.IMAGENET),
    #v2.ColorJitter(brightness=0.2, contrast=0.2, saturation=0.2, hue=0.01),
...
cat_breed_indexes = defaultdict(list)
dog_breed_indexes = defaultdict(list)
for index, (input, label) in enumerate(train_set):
    if train_dataset.classes[label] in cat_breeds:
        cat_breed_indexes[train_dataset.classes[label]].append(index)
    else: 
        dog_breed_indexes[train_dataset.classes[label]].append(index)

cat_breed_count = defaultdict(int)
for breed in cat_breed_indexes:
    cat_breed_count[breed] = len(cat_breed_indexes[breed])

indexes_to_keep = []
for cat_breed, indexes in cat_breed_indexes.items():
    number_to_keep = int(0.1 * cat_breed_count[cat_breed])
    keep_index = random.sample(indexes, number_to_keep)
    indexes_to_keep.extend(keep_index)

for dog_breed, indexes in dog_breed_indexes.items():
    indexes_to_keep.extend(indexes)

train_set_imbalanced = Subset(train_set, indexes_to_keep)

class_frequencies = defaultdict(int)
for input, label in train_set_imbalanced:
    class_frequencies[label] += 1

weights = []
for label in range(37):
    weights.append(1/class_frequencies[label])

weights = torch.tensor(weights)
normalized_weights = weights / weights.sum() * 37
normalized_weights.to(device)


# Loaders
train_loader = DataLoader(
    train_set_imbalanced, batch_size=32, shuffle=True, num_workers=2, pin_memory=True
)
...
with torch.no_grad():
    for inputs, labels in tqdm(test_loader, desc=f"Final test"):
        ...
        for label, pred in zip(labels, preds):
            if label == pred:
                correct_per_class[train_dataset.classes[label]] += 1
            total_per_class[train_dataset.classes[label]] += 1


test_accuracy = correct / len(test_loader.dataset)

# print per class accuracy
for label, correct_count in correct_per_class.items():
    total_count = total_per_class[label]
